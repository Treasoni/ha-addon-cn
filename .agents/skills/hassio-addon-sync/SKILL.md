...
基线在 `addons-manifest.json`（提交入库）。同步脚本：`.codex/scripts/sync-addons.py`（无第三方依赖，`python` 直接跑）。

**镜像地址重写**：本商店面向国内用户，所有 vendored add-on 的 `config.yaml` `image:` 会由
同步脚本在每次同步时自动改写为国内镜像源（`ghcr.nju.edu.cn`，post-sync transform，幂等）。
规则与验证方法见 `.codex/rules/common/mirror-sources.md`；全量校验用
`python .codex/scripts/check-images.py`（发布前跑一次）。不要手工改 config.yaml 的 image。
...
- **查看状态**：看 manifest、冲突、缺哪些中文指南。 → `python .codex/scripts/sync-addons.py --zh-status` / `--readme-list`。
...
5. 更新 manifest：
   ```
   python .codex/scripts/sync-addons.py --zh-status   # 确认该 slug 进入"已有"清单
   ```
...
```
python .codex/scripts/sync-addons.py --new-addon <slug> [--name "显示名"] [--version 0.1.0]
```
...
- **完整开发走 `addon-authoring` skill / `addon-authoring-workflow`**（先收集上游资料 → 编写 → `check-addon.py` 门禁 → 审校 → 人工确认 → 发布）。本流程的快速脚手架只建目录，**发布前必须过 `python .codex/scripts/check-addon.py <slug>`**。
