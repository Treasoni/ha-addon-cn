...
The installer writes the registry into `<agent-dir>/platform` and generates discovery paths from its options. It infers `<agent-dir>` from the copied skill location when possible: the shared Codex skill directory maps to the Codex layout, `.agents/skills` maps to `.claude`, and `.agent/skills` maps to `.agent`. For a custom runtime directory, run the installer with `--agent-dir` and optional `--skills-dir`, `--workflows-dir`, `--subagents-dir`, `--hooks-dir`, and `--hooks-config`.
