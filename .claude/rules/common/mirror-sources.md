...
---
paths:
  - ".claude/scripts/registry_mirror.py"
  - ".claude/scripts/rewrite-images.py"
  - ".claude/scripts/check-images.py"
  - ".claude/scripts/sync-addons.py"
---
...
- **全量校验**：`python .claude/scripts/check-images.py`（192 个逐个用真实 version 探测，
...
1. `python .claude/scripts/check-images.py` 确认哪个源失效。
2. 验证候选源（见上「如何验证」），追加进 `KNOWN_MIRRORS` 与本表。
3. `python .claude/scripts/rewrite-images.py`（自动探测选源，或 `--mirror` 指定）改完全部。
4. `python .claude/scripts/check-images.py` 复核全绿。
...
python .claude/scripts/rewrite-images.py
