...
- 工作流定义：`.claude/workflows/zh-guide-workflow/`
- 状态文件：`workspace/workflow-runs/zh-guide-batch-{run_id}.workflow.md`
- 门禁脚本：`.claude/scripts/zh-guide-gate.py`
...
### P0 规划与枚举
1. 读 `.claude/rules/workflow-routing.md`、`.learnings/RULES.md`、`LEARNINGS.md`、`ERRORS.md`。
...
5. 与用户确认：范围（全部 vs `.claude/skills/hassio-addon-sync/guides/priority-list.md` 子集）、严格度（strict vs 宽松）。按本仓库 grilling 约定问清再继续。

### P1 同步与基线
- 状态文件 frontmatter 记录 `baseline_commit`（前置提交的 sha）。
- 运行 `python .claude/scripts/sync-addons.py`（预热 `.cache/upstream/{源}`，对账 manifest；README 永不覆盖，安全）。`.cache/` 已 gitignore。
- 确认无 vendored 目录 dirty（dirty 目录阻塞该 slug 生成，记入 `## 异常记录`）。

### P2 批量生成
- 排序：`.claude/skills/hassio-addon-sync/guides/priority-list.md` 成员在前 → 按源优先级（alexbelgium→official→frenck）→ 字母序。
...
### P3 结构门禁
- `PYTHONIOENCODING=utf-8 python .claude/scripts/zh-guide-gate.py --batch --strict`。
...
### P6 收尾发布
- `python .claude/scripts/zh-guide-gate.py --mark-zh <通过slugs...>`（脚本复验磁盘标记，无标记拒绝）。
- `python .claude/scripts/sync-addons.py --zh-status` 验证计数（45 → 45+N）。
...
- phase 状态**只**通过 `.claude/scripts/todo-state.sh` 变更：`start|complete|skip|block <P#n> ["reason"]`。
