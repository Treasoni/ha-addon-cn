...
1. 运行同步脚本（会浅克隆/更新 `.cache/upstream/{源}`，目录已 gitignore）：
   ...
   ```
   想先看变更不动文件：加 `--dry-run`。
2. **审查变更摘要**：新增/更新/未变化/跳过/删除/冲突 各自数量与名单；`skipped` 里的 add-on 有未提交本地修改（含中文 README），不要覆盖。
3. 有变更则提交：
   ```
   git add -A
   git commit -m "sync: update add-ons from upstream (N updated, M added, K conflicts)"
   ```
4. **推送两个远程**（GitHub `origin` + Gitee `gitee`）：
   ```
   git push origin main
   git push gitee main
   ```
   认证失败（HTTPS/SSH 凭据）时不要反复重试，把命令与原因告诉用户，由用户手动推。
5. 若脚本输出「缺中文指南」清单，按需调用「中文指南流程」。
...
6. **不修改上游 add-on 的内部文件内容**（只做整目录镜像与 README 本地化），除非是用户自有 add-on。
