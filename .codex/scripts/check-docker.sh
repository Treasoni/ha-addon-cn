...
usage() {
  cat <<'USAGE'
Usage:
  .codex/scripts/check-docker.sh [--path FILE|DIR]... [--all] [--baseline FILE] [--addon-consistency]

Checks Dockerfile / build.json / build.yaml / config.yaml / .dockerignore against
.codex/rules/common/dockerfile.md (通用铁律 + add-on 镜像约定).
...
SCAFFOLD_DOCKERFILE="${PROJECT_ROOT}/.agents/skills/hassio-addon-sync/templates/new-addon/Dockerfile"
