...
- Follow `.codex/rules/common/dockerfile.md` whenever creating, updating, or auditing `Dockerfile`, `build.json`, `build.yaml`, `.dockerignore`, or docker-compose files.
- Reuse templates under `.claude/templates/docker/` for new Dockerfiles; add-on Dockerfiles must stay byte-identical to `.agents/skills/hassio-addon-sync/templates/new-addon/Dockerfile`.
